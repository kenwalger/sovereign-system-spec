...
* [sovereign-sdk-core]({{ site.baseurl }}/sdk/sovereign-sdk-core.html) — Foundational provenance engine, payload sieving, schema validation, and cryptographic signing primitives.
* [sovereign-sdk-fastapi]({{ site.baseurl }}/sdk/sovereign-sdk-fastapi.html) — FastAPI and ASGI middleware supporting transparent auditing and forensic receipt generation.
* [sovereign-sdk-sieve]({{ site.baseurl }}/sdk/sovereign-sdk-sieve.html) — Zero-dependency utility implementing the Sieve-and-Sign Pattern to reduce Prose Tax and enforce ingestion boundaries.
* [sovereign-sdk-ledger]({{ site.baseurl }}/sdk/sovereign-sdk-ledger.html) — Cryptographically verifiable reasoning ledgers and forensic receipt generation.
* [sovereign-sdk-sensor]({{ site.baseurl }}/sdk/sovereign-sdk-sensor.html) — Lightweight telemetry, event capture, and trusted data acquisition for sovereign edge networks.
* [sovereign-sdk-edge]({{ site.baseurl }}/sdk/sovereign-sdk-edge.html) — Edge-node execution, inference routing, and local-first workload orchestration across sovereign compute boundaries.
...
- **[Write-Side Custody]({{ site.baseurl}}/terms/write-side-custody.html):** The architectural discipline of governing admission to durable state: whether a proposed write may become durable memory at all, who or what is authorized to assert it, and what provenance must accompany it. It is not a truth-detection mechanism; a claim may be true and still inadmissible if its source lacks authority to establish it. Custody enforces; the reasoning ledger witnesses.
  - **Origin:** First formalized in the Sovereign Systems Specification by Ken W. Alger, 2026.
...
- **[Active Working Memory]({{ site.baseurl}}/terms/active-working-memory.html):** The assembled execution state that supports a single task, sitting between durable storage and the context window. It is the RAM layer of an agentic memory hierarchy: temporary, task-scoped, and deliberately assembled from retrieved records, tool responses, session state, and planner output. Active Working Memory decides what matters *now*, where the context window only presents the result and durable memory only preserves what may matter later. Failures often blamed on the model, such as stale records, missing constraints, or a window flooded with weakly relevant prose, are usually failures of this layer. * **Origin:** First formalized in the Sovereign Systems Specification by Ken W. Alger, 2026.
- **[Durable Memory]({{ site.baseurl}}/terms/durable-memory.html):** The layer that decides what an agentic system intentionally preserves beyond a single task. It is not storage. Storage keeps whatever it is given; Durable Memory keeps only what is authoritative, verified, and worth carrying into future reasoning. A vector database is one possible implementation, not the definition. The distinction it enforces is the difference between institutional knowledge and institutional folklore.
  - **Origin:** First formalized in the Sovereign Systems Specification by Ken W. Alger, 2026.
